...
function sync() {
    ...
    arr=($1 )
    skopeo --debug sync --multi-arch -f v2s1 --src docker --dest docker "$SOURCE/${arr[0]}" "$DESTINATION/${arr[1]}"
    # 判断是否执行成功
    if [ $? -ne 0 ]; then
...
function copy() {
    echo "::group::Coping $1"
    arr=($1 )
    skopeo --debug copy --multi-arch -f v2s1 "docker://$SOURCE/${arr[0]}" "docker://$DESTINATION/${arr[1]}"
    # 判断是否执行成功
    if [ $? -ne 0 ]; then
